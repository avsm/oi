...
(** [with_attempts ~label ?max_attempts ?initial_delay_s f] runs [f ()] and
    ...
    last exception is re-raised so the caller can surface a real error. Each
    retry logs a warning under the [oi.retry] log source citing [label] so
    operators can see what flaked.
