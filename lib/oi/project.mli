...
type t = {
  deps : string list;
      (** Direct deps, sorted, deduplicated. Excludes local packages
          and "ocaml". *)
  local_packages : string list;
      (** Names of *.opam files in the dir (without the [.opam] suffix). *)
  extra_repos : extra_repo list;
      (** Union of [x-opam-repositories:] entries across all *.opam,
          in declared order (first-occurrence wins on duplicate names). *)
  pins : pin list;
      (** Union of [pin-depends:] entries across all *.opam, in
          declared order. *)
}
...
(** [load ~fs dir] scans every [*.opam] file in [dir]. Raises
    ...
    conflicting [extra_repos] (same name, different URL), or on
    conflicting [pins] (same package name, different URL or version). *)
