...
module Script = struct
  ...
  (* Minimal opam file whose [depends:] is exactly the script's parsed
     deps (same set [oi run] solves). [oi build]'s project mode keys off a
     [*.opam] file — without one there is nothing for it to solve. *)
  let opam_of_deps ~script deps =
    let buf = Buffer.create 256 in
    Buffer.add_string buf "opam-version: \"2.0\"\n";
    Buffer.add_string buf
      (Printf.sprintf "synopsis: \"Generated by oi for %s\"\n"
  ...
  (* Scaffold a [dune-project] + [dune] + [<base>.opam] for an in-place
     [.ml] script so an editor / LSP sees a real dune project AND
     [oi build] has an opam file to solve. The script is left as-is (not
     copied to [main.ml], so LSP resolves the user's own file) and the
     executable's [(name …)] is the script's basename, so dune compiles
     the script itself. Each file is written only if absent — existing
     project files are NEVER overwritten. Returns [(created, kept)]: the
     basenames written vs. the ones left untouched. *)
  let scaffold_for_script ~script ~deps ~dir =
    let base = Filename.remove_extension (Filename.basename script) in
    let outcome name content =
      let p = dir / name in
      if Sys.file_exists p then `Kept name
      else begin
        Out_channel.with_open_text p (fun oc -> output_string oc content);
        `Created name
      end
    in
    let results =
      [
        outcome "dune-project" "(lang dune 3.0)\n";
        outcome "dune" (dune_stanza_of_deps ~name:base deps);
        outcome (base ^ ".opam") (opam_of_deps ~script deps);
      ]
    in
    let created =
      List.filter_map
        (function `Created n -> Some n | `Kept _ -> None)
        results
    in
    let kept =
      List.filter_map
        (function `Kept n -> Some n | `Created _ -> None)
        results
    in
    (created, kept)
end
