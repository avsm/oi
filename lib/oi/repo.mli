(** Opam repository management.
    ...
    {!refresh_max_age} is re-pulled on next use. Pass [~refresh:true] to force
...
(** Seconds. Default [86_400.0] (24h). A repo clone older than this is
    pulled again on next use. *)
...
val ensure_extra :
  ...
  string list
(** [ensure_extra ~data_dir extras] clones/updates each entry using the
