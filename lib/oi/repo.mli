(** Opam repository management.
    ...
    Wraps opam's {!OpamRepository} backend for cloning and refreshing
    individual opam-repository remotes. Higher-level logic (the set
    of "base" repositories, project extras, and overlay resolution)
    lives in {!Reporepo} and the CLI surface. *)
...
(** [cache_fresh ~refresh ~sentinel ~max_age] is [true] when a cache entry
    ...
    [true]. Shared by {!Pin} and {!Url_project} so their sentinel-based TTL
    policies stay in lockstep. *)
...
(** Low-level clone/update primitive shared with the reporepo materialiser.
    ...
    true or the clone is older than {!refresh_max_age}. *)
