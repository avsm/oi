...
val ensure :
  ...
  wrote_by:string ->
  unit
(** [ensure ~fs ~cache_root ~os_key ~wrote_by] writes (or refreshes) the pointer
    under [cache_root]'s [os_key/registry.json]. Idempotent: skips when an
    existing file already matches the current schema values.
