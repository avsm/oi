...
val ensure :
  ...
  wrote_by:string ->
  unit
(** [ensure] writes (or refreshes) the pointer under [cache_root]'s
    [os_key/registry.json]. Idempotent: skips when an existing file already
    matches the current schema values. *)
