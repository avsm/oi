...
type info = {
  ...
  relocatable : bool;
      (** [true] when the toolchain's compiler can be installed into a per-solve
          ...
          oxcaml) means [oi] from-source-builds {!root_names} as a unit into
          {!install_prefix} (a user-writable [$XDG_CACHE_HOME] location),
          PATH-/[OCAMLPATH]-layers that prefix into every consumer build, and
          treats those packages as virtually-installed for solver purposes. *)
  preinstalled_override : bool option;
      (** Override for the [ocaml:preinstalled] opam variable inside
          {!Solver.Ctx.create}. [None] for normal consumer builds (preinstalled
          follows [relocatable]). {!Toolchain_install} sets [Some false] for the
          one-off toolchain build so [ocamlfind+ox]'s configure does not pass
          [-no-topfind] — the toolchain prefix is user-writable, so [topfind]
          can actually be installed there. *)
...
(* The toolchain is ready when its install prefix has a working compiler at
   [bin/ocamlc]. For relocatable toolchains the compiler builds into the
   consumer prefix on every solve, so [install_prefix] is just a synthetic
   identity and [is_ready] is always true. *)
let external_ready p = Sys.file_exists (p / "bin" / "ocamlc")
let is_ready info = info.relocatable || external_ready info.install_prefix
...
let opam_ctx_of_info (info : info) : Solver.Ctx.toolchain =
  {
    ...
    preinstalled_override = info.preinstalled_override;
...
(* Source-built toolchains live under $XDG_CACHE_HOME (the relocatable
   compiler builds the consumer solves do directly) — these are cache
   artefacts, derived from the solve. *)
...
(* Derive the install-prefix hash. Combines [D10.Layer.hash] (which hashes
   the solved packages' opam [effective_part]) with the conf's platform
   fields. Same set of inputs that drive opam-0install's solve, so any
   change → fresh install_prefix → old prefix is left alone. *)
...
(* Toolchain prefix derivation.

   For non-relocatable toolchains [oi] always source-builds the toolchain
   roots into a single deterministic location under [$XDG_CACHE_HOME].
   This replaces the prior env / brew / [x-oi-external-prefix] / [/opt]
   probe chain — the new model is "[oi] owns the toolchain prefix, always",
   which sidesteps the read-only-system-stdlib + [-no-topfind] issue that
   blocked any [ocamlfind+ox] install at [/opt/oxcaml/<ver>].

   For relocatable toolchains [install_prefix] is a synthetic identity:
   the compiler builds into each consumer prefix directly, the value just
   keys cache lookups. *)
let derive_install_artifacts ~conf ~handle ~relocatable ~pkgs ~packages_dirs
    ~ocaml_version =
  ...
  let install_prefix =
    if relocatable then default_root () / dir_name / "_opam"
    else default_root () / dir_name
  in
  (hash, install_prefix)
...
let resolve ~fs ~sys ~data_dir:_ ~(conf : Solver.Ctx.conf) ~handle =
  ...
  let hash, install_prefix =
    derive_install_artifacts ~conf ~handle ~relocatable ~pkgs ~packages_dirs
      ~ocaml_version
  in
  let compiler_name =
...
(* -- Probe and auto-install --------------------------------------------- *)

(* For non-relocatable toolchains, the actual install is driven by
   {!Toolchain_install.ensure} via the {!Build_pipeline.aux_installer}
   plumbing. {!ensure_installed} only handles the relocatable / already-ready
   cases; the install itself sits one layer up so it can reach
   {!Build_pipeline.solve} without a module cycle. *)
let ensure_installed ?(reporter = Build_progress.null) (info : info) =
  if info.relocatable then
    Log.debug (fun m ->
        m
          "toolchain %s is relocatable — compiler builds into the consumer \
           prefix"
          info.handle)
  else if external_ready info.install_prefix then
    Fmt.kstr
      (fun s -> reporter.Build_progress.event (Status s))
      "Using toolchain %s at %s" info.handle info.install_prefix
  else
    Log.debug (fun m ->
        m
          "toolchain %s: prefix %s not yet populated — {Toolchain_install} \
           will source-build it"
          info.handle info.install_prefix)
