...
(* Multi-stage build:
     [build] stage: install toolchain + project depexts; lay out the .orig
       tarball + debian/ tree; run [dpkg-buildpackage -b -uc -us].
     final stage: [scratch] copying just the produced .deb so
       [docker buildx build --output type=local] hands the user only the
       artefact, not the whole build image. *)
let dockerfile (s : Spec.t) (t : Target.t) ~overlay_depexts =
  ...
  @@ DF.run "chmod 0755 debian/rules"
  @@ DF.run "dpkg-buildpackage -b -uc -us"
  @@ DF.comment "=== final: just the .deb ==="
  @@ DF.from "scratch"
  @@ DF.copy ~from:"build" ~src:[ "/work/*.deb" ] ~dst:"/" ()
