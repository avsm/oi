...
(* Group succeeded layers by their overlay tag. Layers with no overlay
   are omitted. Returns one bucket per (handle, version) tuple, keyed
   additionally by os_key so each bucket can be written to the right
   per-overlay subdir. *)
let layers_by_overlay (c : Config.t) =
  let layers_dir = Eio.Path.(c.root / "layers") in
  if not (Sysops.file_exists layers_dir) then []
  else
    let tbl : (string * string * string, string list) Hashtbl.t =
      Hashtbl.create 16
    in
    let os_keys = Eio.Path.read_dir layers_dir in
    List.iter
        ...
        List.iter
          (fun h ->
            if String.contains h '.' then ()
              ...
              let meta_path = Eio.Path.(os_layer_dir / h / "layer.json") in
              match load_meta meta_path with
              | Some m
                when m.exit_status = 0
                     && m.overlay_handle <> None
                     && m.overlay_version <> None ->
                  let handle = Option.get m.overlay_handle in
                  let version = Option.get m.overlay_version in
                  let key = (handle, version, os_key) in
                  let prev = try Hashtbl.find tbl key with Not_found -> [] in
                  Hashtbl.replace tbl key (h :: prev)
              | _ -> ())
          hashes)
      os_keys;
    Hashtbl.fold
      (fun (h, v, os) hashes acc -> (h, v, os, hashes) :: acc)
      tbl []

(* Hardlink [src] to [dst]. Falls back to copy on EXDEV (cross-device)
   or when hardlinks are unsupported. Returns [true] on success. *)
let hardlink_or_copy ~src ~dst =
  try
    Unix.link src dst;
    true
  with
  | Unix.Unix_error (Unix.EEXIST, _, _) -> true
  | Unix.Unix_error _ -> (
      try
...
(* Populate per-overlay subtrees under [dst] by hardlinking the
   [<dst>/<os_key>/<hash>.tar.zst] archives (already emitted by
   {!export_all}) into [<dst>/<handle>.<version>/<os_key>/]. Writes
   an OINDEX.txt for each overlay/os_key pair. [handles] optionally
   restricts the output to a subset of overlay handles.

   Returns the number of newly-linked archives. *)
let export_per_overlay (c : Config.t) ~dst ?handles () =
  let buckets = layers_by_overlay c in
  let matches handle =
    match handles with None -> true | Some hs -> List.mem handle hs
  in
  let count = ref 0 in
  let touched_dirs : (string * string, unit) Hashtbl.t = Hashtbl.create 8 in
  List.iter
    (fun (handle, version, os_key, hashes) ->
      if matches handle then begin
        let overlay_root =
          Eio.Path.(dst / Fmt.str "%s.%s" handle version)
        in
        let os_dir = Eio.Path.(overlay_root / os_key) in
        Eio.Path.mkdirs ~exists_ok:true ~perm:0o755 os_dir;
        List.iter
          (fun h ->
            let src =
              Eio.Path.native_exn
                Eio.Path.(dst / os_key / (h ^ ".tar.zst"))
            in
            let dst_f =
              Eio.Path.native_exn Eio.Path.(os_dir / (h ^ ".tar.zst"))
            in
            if Sys.file_exists src && not (Sys.file_exists dst_f) then begin
              if hardlink_or_copy ~src ~dst:dst_f then incr count
            end)
          hashes;
        Hashtbl.replace touched_dirs (Fmt.str "%s.%s" handle version, os_key) ()
      end)
    buckets;
  Hashtbl.iter
    (fun (overlay_dir, os_key) () ->
      let overlay_root = Eio.Path.(dst / overlay_dir) in
      write_index ~dst:overlay_root os_key)
    touched_dirs;
  !count
