...
(* Standalone build of an [oi dist makefile] bundle. Two stages, no [oi]
   or [opam] at build time — only the emitted Makefile + recipes:

   - [build]: install the host build toolchain ([build_depexts]) plus the
     closure's [overlay_depexts] (and the distro OCaml package iff the
     plan has host-provided compiler layers), copy the whole bundle in,
     and run [make install] (which depends on [all], so it fetches the
     d10ir archives from the Makefile's baked-in REGISTRY and builds the
     whole closure) into [/dist/usr/local].
   - runtime: a clean distro image carrying only the libraries the
     binaries dlopen at run time (the overlay depexts again) plus the
     [/dist/usr/local] tree — i.e. just the selected package(s)'
     [bin/]+[sbin/]+[share/]. *)
(* The build-stage and runtime-stage package sets for a makefile
   Dockerfile, plus the toolchain-provenance note. Build = host
   toolchain ([build_depexts]) ∪ [overlay_depexts] ∪ (distro [ocaml] iff
   [needs_host_ocaml]); runtime = just [overlay_depexts] (the libs the
   binaries dlopen). Split out so [dockerfile_makefile] stays short. *)
let makefile_depext_plan ~mgr ~needs_host_ocaml ~overlay_depexts =
  let base = build_depexts mgr in
  let base_words =
    String.split_on_char ' ' base |> List.filter (fun s -> s <> "")
  in
  (* The distro OCaml package name is [ocaml] on apt/dnf/apk alike. *)
  let ocaml_pkg = if needs_host_ocaml then [ "ocaml" ] else [] in
  let extras =
    overlay_depexts @ ocaml_pkg
    |> List.filter (fun p -> not (List.mem p base_words))
    |> List.sort_uniq String.compare
  in
  let build_pkgs =
    match extras with [] -> base | xs -> base ^ " " ^ String.concat " " xs
  in
  let runtime_pkgs =
    String.concat " " (List.sort_uniq String.compare overlay_depexts)
  in
  let toolchain_note =
    if needs_host_ocaml then
      ...
      "Self-contained: the OCaml compiler is built from source (no host \
       toolchain needed)."
  in
  (build_pkgs, runtime_pkgs, toolchain_note)

let dockerfile_makefile ~label ~needs_host_ocaml ?(overlay_depexts = []) d =
  let resolved = Distro.resolve_alias d in
  let img, tag = Distro.base_distro_tag (resolved :> Distro.t) in
  let mgr = Distro.package_manager (resolved :> Distro.t) in
  let human = Distro.human_readable_string_of_distro (resolved :> Distro.t) in
  (* Same as [one_distro_filename] (defined below — inlined to avoid a
     forward reference); only used in the header comment. *)
  let dockerfile_name =
    "Dockerfile." ^ Distro.tag_of_distro (resolved :> Distro.t)
  in
  let build_pkgs, runtime_pkgs, toolchain_note =
    makefile_depext_plan ~mgr ~needs_host_ocaml ~overlay_depexts
  in
  DF.comment "Generated by `oi dist makefile` -- standalone build of %s on %s."
    label human
  @@ DF.comment "Build: docker build -t my-project -f %s ." dockerfile_name
  @@ DF.comment "%s" toolchain_note
  @@ DF.comment "=== Stage: build (%s) ===" human
  @@ DF.from ~alias:"build" ~tag img
  @@ DF.env [ ("CI", "true") ]
  @@ DF.run "%s" (install_cmd mgr build_pkgs)
  @@ DF.workdir "/oi-build"
  @@ DF.copy ~src:[ "." ] ~dst:"/oi-build" ()
  @@ DF.run "make install PREFIX=/usr/local DESTDIR=/dist"
  @@ DF.comment "=== Stage: runtime (clean %s + just the binaries) ===" human
  @@ DF.from ~tag img
  @@ DF.env [ ("CI", "true") ]
  @@ (if runtime_pkgs = "" then DF.empty
      else DF.run "%s" (install_cmd mgr runtime_pkgs))
  @@ DF.copy ~from:"build" ~src:[ "/dist/usr/local/" ] ~dst:"/usr/local/" ()
