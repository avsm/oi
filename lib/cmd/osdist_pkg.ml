...
(* Pluck the overlay depexts for [t] from the sidecar by Distro.tag. Static
   targets get empty (the alpine build doesn't need distro depexts). *)
let depexts_for (s : Osdist.Spec.t) (t : Osdist.Target.t) =
  match t.distro with
  | None -> []
  | Some d ->
      let tag = Dockerfile_opam.Distro.tag_of_distro d in
...
let materialise_target ~out ~bundle_tarball ~(spec : Osdist.Spec.t)
  ...
  | Osdist.Target.Static ->
      let df = Osdist.Alpine_static.dockerfile spec t in
      render_dockerfile (dir / "Dockerfile") df;
      write_file
        ~path:(dir / "build.sh")
...
(* Static preamble of the top-level build.sh: defines the [build_one]
   shell function which docker-builds one target subdir, accumulates
   pass/fail state, and never aborts the outer loop on a single target
   failure. *)
let build_sh_preamble =
  {|#!/bin/sh
# Generated by `oi dist pkg`. Loops `docker buildx build` over every
# target subdir, dropping artefacts into <out>/artefacts/<tag>/.
#
...
build_one() {
  ...
  echo "==> building $t"
  rm -rf "$ART/$t"; mkdir -p "$ART/$t"
  if ( cd "$HERE/$t" && docker buildx build \
         --output type=local,dest="$ART/$t" \
...
(* compose.yaml — parallel counterpart to build.sh. Drive it with either:

     docker buildx bake -f compose.yaml         # parallel build + extract
                                                # files into ./artefacts/<tag>/
     docker compose -f compose.yaml build       # parallel image build only

   We rely on buildx-bake's [x-bake:] compose extension: bake reads the
   compose file, honours [x-bake:] per service to pull in buildx fields
   ([output], [cache_from], …) that the plain compose schema doesn't yet
   accept, and runs every target in parallel. [docker compose build]
   itself just ignores [x-bake:] and builds the images without
   extraction — a useful fallback when you only want images. *)
...
#
#   docker buildx bake -f compose.yaml     # parallel build + extract artefacts
#                                          # straight into ./artefacts/<tag>/
#                                          # via the per-service x-bake: block
#   docker compose -f compose.yaml build   # parallel image build only
#
...
let pkg_run (c : Terms.common) refresh registry use_registry with_repos
  ...
  Oi.Say.info "  parallel (fail-fast):   docker buildx bake -f %s" compose;
  (* Stage 3 (optional): docker-build every target. *)
  if build_flag then begin
    Oi.Say.step "invoking %s" top;
    cmd_run_inherit ~sys:harness.Harness.sys [ top ];
...
let pkg_man =
  [
    ...
    `P
      "Build the project's source bundle and materialise the per-distro \
       packaging tree (Dockerfiles, $(b,debian/) trees, $(b,.spec) files) \
       in one step.";
    `P
      "With no $(i,TARGET), run from a project dir: the root opam package \
       (the one at the top of the project's $(b,*.opam) dependency DAG) is \
       used as the package name. Otherwise the first $(i,TARGET) argument \
       names the package.";
    `P
      "With $(b,--build), also drive $(b,docker buildx build) for every \
       target distro and collect the artefacts under \
       $(b,<output>/<tag>/artefacts/).";
    `P
      "Output layout under $(b,-o DIR):";
    `Pre
      "  DIR/bundle/<pkg>-<ver>.tar.gz          source tarball\n\
      \  DIR/bundle/<pkg>-<ver>.tar.gz.sha256   sha256 sidecar\n\
      \  DIR/bundle/<pkg>-<ver>.osdist.json     Spec sidecar (metadata + depexts)\n\
      \  DIR/<tag>/Dockerfile, debian/ | .spec  per-target packaging\n\
      \  DIR/build.sh                            sequential, resilient docker buildx loop\n\
      \  DIR/compose.yaml                        parallel `docker buildx bake` (or `compose build`)";
    `S Manpage.s_examples;
    `Pre
      "  oi dist pkg -o ./pkg --gpg-key 0xDEADBEEF --epoch 1\n\
      \  oi dist pkg -o ./pkg --gpg-key 0xDEADBEEF --epoch 1 --build\n\
      \  oi dist pkg utop -o ./pkg --build         # package an upstream opam target";
  ]
...
let pkg_cmd =
  ...
  let cli_targets =
    Arg.(
      ...
      & info ~docv:"TARGET"
          ~doc:
            "Package name (opam target). Omit to use the cwd project's root \
             opam package."
          [])
  in
  let pkg_name =
  ...
  let version =
    Arg.(
      ...
      & info ~docv:"VER"
          ~doc:
            "Override the derived version (defaults to dune-project's \
             $(b,version) or the opam $(b,version:) field)."
          [ "pkg-version" ])
  in
  let epoch =
    Arg.(
      ...
      & info ~docv:"N"
          ~doc:
            "Package epoch (Debian $(b,N:) prefix / RPM $(b,Epoch:) tag). \
             Bump when a version needs to sort above an earlier release."
          [ "epoch" ])
  in
  let maintainer =
    Arg.(
      ...
      & info ~docv:"NAME <EMAIL>"
          ~doc:"Maintainer field. Defaults to \\$DEBFULLNAME <\\$DEBEMAIL>."
          [ "maintainer" ])
  in
  let homepage =
    Arg.(
  ...
  let prefix =
    Arg.(
      value
      & opt (some string) None
      & info ~docv:"PATH" ~doc:"Install prefix (defaults to $(b,/usr))."
          [ "prefix" ])
  in
  let distros =
    Arg.(
      ...
      & info ~docv:"LIST"
          ~doc:
            "Comma-separated subset of target distros to emit (e.g. \
             $(b,ubuntu-26.04,fedora-44)). Default: every entry in \
             $(b,Osdist.Target.default_targets)."
          [ "distros" ])
  in
  let build_flag =
    Arg.(
      ...
      & info
          ~doc:
            "After materialising the tree, also run the emitted \
             $(b,build.sh) to docker-build every target."
          [ "build" ])
  in
  Cmd.v
    (Cmd.info "pkg"
       ~doc:"Build source bundle + per-distro packaging tree in one step"
       ~man:pkg_man)
    Term.(
