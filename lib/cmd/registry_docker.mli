(** Dockerfile generation for [oi docker].
    ...
    [s3cmd put --recursive --skip-existing /registry/ s3://oiu/] into its build,
    using BuildKit secrets to supply the S3 credentials. The parallel obuilder
    spec emitter ({!obuilder_spec_one_distro}) drives the same flow under
...
type s3_config = {
  bucket : string;
      (** Target URI for the final [s3cmd put --recursive --skip-existing] (e.g.
          [s3://oiu/]). *)
...
val dockerfile_makefile :
  label:string ->
  needs_host_ocaml:bool ->
  ?overlay_depexts:string list ->
  Distro.t ->
  Dockerfile.t
(** [dockerfile_makefile ~label ~needs_host_ocaml ?overlay_depexts d] is a
    two-stage Dockerfile that builds an [oi dist makefile] bundle standalone —
    no [oi]/[opam] at build time, only the emitted Makefile + recipes. The
    [build] stage installs {!build_depexts} ∪ [overlay_depexts] (∪ the distro
    [ocaml] package iff [needs_host_ocaml], i.e. the plan has host-provided
