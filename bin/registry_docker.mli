(** Dockerfile generation for [oi registry docker].
    ...
    [oi registry export /out]. A [docker-compose.yml] orchestrates them with
    a shared host bind mount at [/out] so a single [docker compose up] runs
    every distro in parallel and leaves the exported registry on the host. *)
...
(** [dockerfile_oi ~src_context] emits a Dockerfile that builds a static
    musl-linked [oi] binary from the source tree at [src_context] (relative
    to the build context) and exports it as a scratch image at [/oi]. *)

val dockerfile_one_distro :
  src_context:string ->
  packages_ctx_path:string ->
  Distro.t ->
  Dockerfile.t
(** Per-distro build image. Stage 0 is the [oi-builder]; the final stage
    installs depexts, copies the static [oi] and [packages.txt] into place,
    and sets a [CMD] that runs [oi registry build] followed by
...
(** Read a packages file at [path]: strip [#] comments and blank lines,
    return one opam target spec per entry. *)
