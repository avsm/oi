...
(* Each distro stage installs depexts and the static oi binary but does
   ...
   [command:] that invokes [oi registry build --all] (which iterates
   every overlay's [x-root-packages] in the bind-mounted reporepo) plus
   the final [oi registry export]. Keeping the image generic lets the
   same image be re-used across different reporepo pins without
   rebuilding. *)
let distro_stage d =
...
let dockerfile_one_distro ~src_context d =
  let resolved = Distro.resolve_alias d in
