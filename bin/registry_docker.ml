...
(* Each distro stage installs depexts and the static oi binary but does
   not run the build/export itself — the compose file supplies a
   [command:] that chains one or more [oi registry build] passes plus
   the final [oi registry export]. Keeping the image generic lets the
   same image be re-used across different overlay sets without
   rebuilding. *)
...
let dockerfile_one_distro ~src_context ~packages_ctx_path d =
  ...
  @@ DF.comment
       "Usage: docker compose up  (mounts ./registry on /out and runs oi \
...
(* Shell one-liner that a distro service runs. First pass is the
...
let build_export_cmd ~overlays =
  let main_pass = "oi registry build $(cat /work/packages.txt)" in
  let overlay_pass handle = Printf.sprintf "oi registry build @%s" handle in
  let overlay_passes = List.map overlay_pass overlays in
  String.concat " && "
    ((main_pass :: overlay_passes) @ [ "oi registry export /out" ])

let docker_compose_yaml ?(overlays = []) ~distros ~registry_host_path () =
  let buf = Buffer.create 1024 in
  let out s = Buffer.add_string buf s in
  out "# Generated by `oi registry docker`.\n";
  out "# Run with: docker compose up --build\n";
  out
    "# Each per-distro service runs `oi registry build` + `oi registry export`,\n\
     # writing the per-os_key archives, sqlite index, and OINDEX.txt into the\n\
     # shared bind mount. Each os_key has only one producer container, so no\n\
