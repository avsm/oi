...
(* Shell one-liner that a distro service runs. Each container owns its
   ...
   build iterates every overlay's [x-root-packages] as [@HANDLE/PKG]
   and finishes with [oi registry export /out]. *)
...
let docker_compose_yaml ~distros ~registry_host_path () =
  ...
  out
    "# Each per-distro service runs `oi registry build --refresh --all` —\n\
     # which clones the reporepo into the container's own state dir,\n\
     # iterates every overlay's x-root-packages, and builds each package —\n\
     # then `oi registry export` writes per-os_key archives, the sqlite\n\
     # index, and OINDEX.txt into the shared registry bind mount. Each\n\
     # os_key has only one producer container, so no cross-service merge\n\
     # is needed. Compose exits once every service has finished.\n\
     #\n\
