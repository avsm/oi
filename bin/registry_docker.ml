...
(* Each distro stage installs depexts, then fetches the latest statically
   linked [oi-linux-<arch>] from the [oi] GitHub releases page rather
   than building [oi] from source. This keeps image builds fast and
   makes the release workflow the single source of truth for the [oi]
   binary. The stage doesn't run the build/export itself — the compose
   file supplies a [command:] that invokes [oi registry build --all]
   plus the final [oi registry export]. *)
let distro_stage d =
...
(* Single-distro Dockerfile: one distro stage that curls the [oi] binary
   ...
   build+export. *)
let dockerfile_one_distro d =
  let resolved = Distro.resolve_alias d in
  let tag = Distro.tag_of_distro (resolved :> Distro.t) in
  DF.comment "Generated by `oi registry docker` -- %s registry build image." tag
  @@ DF.comment
       "Usage: docker compose up  (mounts ./registry on /out and the \
        reporepo on /opt/reporepo, then runs oi registry build --all + \
        export)."
  @@ distro_stage d
...
(* Shell one-liner that a distro service runs. [oi registry build --all]
   walks every overlay in the bind-mounted reporepo and builds its
   declared [x-root-packages] as [@HANDLE/PKG] — so there's no package
...
let docker_compose_yaml ~distros ~registry_host_path ~reporepo_host_path () =
  ...
  out
    "# Each per-distro service runs `oi registry build --all` (iterating\n\
     # every overlay's x-root-packages declared in the bind-mounted reporepo)\n\
     # followed by `oi registry export`, writing per-os_key archives, the\n\
     # sqlite index, and OINDEX.txt into the shared registry bind mount. Each\n\
     # os_key has only one producer container, so no cross-service merge is\n\
     # needed. Compose exits once every service has finished.\n";
  out "services:\n";
  ...
  List.iter
    (fun d ->
      ...
      Printf.ksprintf out "    environment:\n";
      Printf.ksprintf out "      OI_REPOREPO: /opt/reporepo\n";
      Printf.ksprintf out "    command: [\"sh\", \"-c\", %s]\n"
        (Printf.sprintf "%S" cmd);
      Printf.ksprintf out "    volumes:\n";
      Printf.ksprintf out "      - %s:/out\n" registry_host_path;
      Printf.ksprintf out "      - %s:/opt/reporepo:ro\n" reporepo_host_path)
    distros;
  Buffer.contents buf
